...
class TaylorSeerCacheConfig:
    ...
    Attributes:
        ...
        taylor_factors_dtype (`ms.Type`, defaults to `ms.bfloat16`):
    ...
    max_order: int = 1
    taylor_factors_dtype: Optional[ms.Type] = ms.bfloat16
...
class TaylorSeerState:
    def __init__(
        ...
        self.module_dtypes: Tuple[ms.Type, ...] = ()
        self.last_update_step: Optional[int] = None
        self.taylor_factors: Dict[int, Dict[int, ms.Tensor]] = {}
    ...
    def update(
        ...
        if self.is_inactive:
            ...
            for i, features in enumerate(outputs):
                new_factors: Dict[int, ms.Tensor] = {0: features}
    ...
    def predict(self) -> List[ms.Tensor]:
        ...
        if self.is_inactive:
            ...
            for i in range(num_outputs):
                ...
                output = mint.zeros_like(taylor_factors[0], dtype=output_dtype)
...
class TaylorSeerCacheHook(ModelHook):
    ...
    def new_construct(self, module: ms.nn.Cell, *args, **kwargs):
        ...
        if should_compute:
            outputs = self.fn_ref.original_construct(*args, **kwargs)
            wrapped_outputs = (outputs,) if isinstance(outputs, ms.Tensor) else outputs
...
def apply_taylorseer_cache(module: ms.nn.Cell, config: TaylorSeerCacheConfig):
    """
    ...
    Args:
        module (ms.nn.Cell): The model subtree to apply the hooks to.
        config (TaylorSeerCacheConfig): Configuration for the cache.

    Example:
    ```python
    >>> import mindspore as ms
    >>> from mindone.diffusers import FluxPipeline, TaylorSeerCacheConfig
    ...
    ...     taylor_factors_dtype=ms.float32,
    ... )
...
def _apply_taylorseer_cache_hook(
    ...
    """
    Registers the TaylorSeer hook on the specified nn.Cell.

    Args:
        name: Name of the module.
        module: The nn.Cell to be hooked.
