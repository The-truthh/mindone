...
from mindone.transformers import Qwen2_5_VLTextModel, SiglipImageProcessor, SiglipVisionModel, T5EncoderModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = HunyuanVideo15ImageToVideoPipeline.from_pretrained(model_id, mindspore_dtype=ms.float16)
...
class HunyuanVideo15ImageToVideoPipeline(DiffusionPipeline):
    ...
    def _get_byt5_prompt_embeds(
        ...
        for glyph_text in glyph_texts:
            if glyph_text is None:
                glyph_text_embeds = mint.zeros(
                    (1, tokenizer_max_length, text_encoder.config.d_model), dtype=text_encoder.dtype
                )
                glyph_text_embeds_mask = mint.zeros((1, tokenizer_max_length), dtype=ms.int64)
            else:
                ...
                glyph_text_embeds_mask = ms.tensor(txt_tokens.attention_mask)

            prompt_embeds_list.append(glyph_text_embeds)
            prompt_embeds_mask_list.append(glyph_text_embeds_mask)

        prompt_embeds = mint.cat(prompt_embeds_list, dim=0)
        prompt_embeds_mask = mint.cat(prompt_embeds_mask_list, dim=0)
    ...
    def _get_image_latents(
        ...
        vae_dtype = vae.dtype
        image_tensor = image_processor.preprocess(image, height=height, width=width).to(dtype=vae_dtype)
        image_tensor = image_tensor.unsqueeze(2)
        image_latents = retrieve_latents(vae, vae.encode(image_tensor)[0], sample_mode="argmax")
    ...
    def _get_image_embeds(
        ...
        image_encoder_dtype = next(image_encoder.get_parameters()).dtype
        image = feature_extractor.preprocess(images=image, do_resize=True, return_tensors="ms", do_convert_rgb=True)
        image = image.to(dtype=image_encoder_dtype)
    ...
    def encode_image(
        ...
        image_embeds = self._get_image_embeds(
            image_encoder=self.image_encoder, feature_extractor=self.feature_extractor, image=image
        )
    ...
    def encode_prompt(
        ...
        prompt_embeds_2 = prompt_embeds_2.to(dtype=dtype)
    ...
    def check_inputs(
        ...
        if prompt_embeds is not None and prompt_embeds_mask is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `prompt_embeds_mask` also have to be passed. Make sure to generate `prompt_embeds_mask` from the same text encoder that was used to generate `prompt_embeds`."  # noqa: E501
            )
        if negative_prompt_embeds is not None and negative_prompt_embeds_mask is None:
            raise ValueError(
                "If `negative_prompt_embeds` are provided, `negative_prompt_embeds_mask` also have to be passed. Make sure to generate `negative_prompt_embeds_mask` from the same text encoder that was used to generate `negative_prompt_embeds`."  # noqa: E501
            )
        ...
        if prompt_embeds_2 is not None and prompt_embeds_mask_2 is None:
            raise ValueError(
                "If `prompt_embeds_2` are provided, `prompt_embeds_mask_2` also have to be passed. Make sure to generate `prompt_embeds_mask_2` from the same text encoder that was used to generate `prompt_embeds_2`."  # noqa: E501
            )
        if negative_prompt_embeds_2 is not None and negative_prompt_embeds_mask_2 is None:
            raise ValueError(
                "If `negative_prompt_embeds_2` are provided, `negative_prompt_embeds_mask_2` also have to be passed. Make sure to generate `negative_prompt_embeds_mask_2` from the same text encoder that was used to generate `negative_prompt_embeds_2`."  # noqa: E501
            )
    ...
    def prepare_cond_latents_and_mask(
        ...
        latent_mask = mint.zeros((batch, 1, frames, height, width), dtype=dtype)
        latent_mask[:, :, 0, :, :] = 1.0
    ...
    def __call__(
        ...
        image_embeds = self.encode_image(
            image=image, batch_size=batch_size * num_videos_per_prompt, dtype=self.transformer.dtype
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = mint.cat([latents, cond_latents_concat, mask_concat], dim=1)
