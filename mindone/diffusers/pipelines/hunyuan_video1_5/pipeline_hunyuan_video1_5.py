...
from mindone.transformers import Qwen2_5_VLTextModel, T5EncoderModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = HunyuanVideo15Pipeline.from_pretrained(model_id, mindspore_dtype=ms.float16)
...
class HunyuanVideo15Pipeline(DiffusionPipeline):
    ...
    def prepare_cond_latents_and_mask(self, latents, dtype: Optional[ms.Type]):
        """
        ...
        cond_latents_concat = mint.zeros((batch, channels, frames, height, width), dtype=dtype)

        mask_concat = mint.zeros((batch, 1, frames, height, width), dtype=dtype)
    ...
    def __call__(
        ...
        cond_latents_concat, mask_concat = self.prepare_cond_latents_and_mask(latents, self.transformer.dtype)
        image_embeds = mint.zeros(
            (batch_size, self.vision_num_semantic_tokens, self.vision_states_dim), dtype=self.transformer.dtype
        )
