...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = OvisImagePipeline.from_pretrained("AIDC-AI/Ovis-Image-7B", mindspore_dtype=ms.bfloat16)
...
class OvisImagePipeline(
    ...
    def __init__(
        ...
        self.system_prompt = "Describe the image by detailing the color, quantity, text, shape, size, texture, spatial relationships of the objects and background: "  # noqa: E501
    ...
    def _get_ovis_prompt_embeds(
        ...
        input_ids = ms.tensor(tokens.input_ids)
        attention_mask = ms.tensor(tokens.attention_mask)
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            prompt_embeds = self._get_ovis_prompt_embeds(prompt=prompt, num_images_per_prompt=num_images_per_prompt)

        dtype = self.text_encoder.dtype if self.text_encoder is not None else self.transformer.dtype
        text_ids = mint.zeros((prompt_embeds.shape[1], 3))
        text_ids[..., 1] = text_ids[..., 1] + mint.arange(prompt_embeds.shape[1])[None, :]
        text_ids[..., 2] = text_ids[..., 2] + mint.arange(prompt_embeds.shape[1])[None, :]
