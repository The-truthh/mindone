...
class MarigoldIntrinsicsOutput(BaseOutput):
    ...
    Args:
        prediction (`np.ndarray`, `ms.Tensor`):
            ...
            height × width` for `ms.Tensor` or `(numimages * numtargets) × height × width × 3` for `np.ndarray`,
        ...
        uncertainty (`None`, `np.ndarray`, `ms.Tensor`):
            ...
            numtargets) × 3 × height × width` for `ms.Tensor` or `(numimages * numtargets) × height × width × 3` for
            `np.ndarray`.
