...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = ZImagePipeline.from_pretrained("Tongyi-MAI/Z-Image-Turbo", mindspore_dtype=ms.bfloat16)
...
class ZImagePipeline(DiffusionPipeline, ZImageLoraLoaderMixin, FromSingleFileMixin):
    ...
    def encode_prompt(
        ...
        prompt_embeds: Optional[List[ms.Tensor]] = None,
        negative_prompt_embeds: Optional[ms.Tensor] = None,
        max_sequence_length: int = 512,
    ):
        prompt = [prompt] if isinstance(prompt, str) else prompt
        prompt_embeds = self._encode_prompt(
            prompt=prompt, prompt_embeds=prompt_embeds, max_sequence_length=max_sequence_length
        )
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_embeds = self._encode_prompt(
                prompt=negative_prompt, prompt_embeds=negative_prompt_embeds, max_sequence_length=max_sequence_length
            )
        else:
    ...
    def _encode_prompt(
        ...
        prompt_masks = ms.tensor(text_inputs.attention_mask).bool()
    ...
    def __call__(
        ...
        prompt_embeds: Optional[List[ms.Tensor]] = None,
        negative_prompt_embeds: Optional[List[ms.Tensor]] = None,
        ...
        Args:
            ...
            prompt_embeds (`List[ms.Tensor]`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `prompt` input argument.
            negative_prompt_embeds (`List[ms.Tensor]`, *optional*):
                ...
                if apply_cfg:
                    ...
                    for j in range(actual_batch_size):
                        ...
                        if self._cfg_normalization and float(self._cfg_normalization) > 0.0:
                            ori_pos_norm = mint.linalg.vector_norm(pos)
                            new_pos_norm = mint.linalg.vector_norm(pred)
                    ...
                    noise_pred = mint.stack(noise_pred, dim=0)
                else:
                    noise_pred = mint.stack([t.float() for t in model_out_list], dim=0)

                noise_pred = noise_pred.squeeze(2)
                noise_pred = -noise_pred

                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred.to(ms.float32), t, latents, return_dict=False)[0]
                assert latents.dtype == ms.float32
