...
class HunyuanVideo15CausalConv3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideo15RMS_norm(nn.Cell):
    r"""
...
class HunyuanVideo15AttnBlock(nn.Cell):
    ...
    def prepare_causal_attention_mask(n_frame: int, n_hw: int, dtype, device, batch_size: int = None):
        ...
        Returns:
            ms.Tensor: Causal attention mask.
        """
    ...
    def construct(self, x: ms.Tensor) -> ms.Tensor:
        ...
        if query.shape[-1] <= 768:
            x = scaled_dot_product_attention(query, key, value, attn_mask=attention_mask)
        else:
            scale_sqrt = float(math.sqrt(query.shape[-1]**-0.5))

            if attention_mask is not None and attention_mask.dtype == ms.bool_:
                attention_mask = mint.logical_not(attention_mask) * dtype_to_min(query.dtype)
            ...
            attention_scores = mint.baddbmm(
                ...
                query * scale_sqrt,
                key.swapaxes(-1, -2) * scale_sqrt,
                beta=1,
                alpha=1,
            )
            attention_probs = mint.softmax(attention_scores, dim=-1)
            x = mint.bmm(attention_probs, value)

            if has_extra_dims:
                x = x.reshape(*origin_query_shape)
...
class HunyuanVideo15Upsample(nn.Cell):
    ...
    def construct(self, x: ms.Tensor):
        ...
        if self.add_temporal_upsample:
            ...
            h = mint.cat([h_first, h_next], dim=2)
            ...
            shortcut = mint.cat([x_first, x_next], dim=2)
...
class HunyuanVideo15Downsample(nn.Cell):
    ...
    def construct(self, x: ms.Tensor):
        ...
        if self.add_temporal_downsample:
            ...
            h_first = mint.cat([h_first, h_first], dim=1)
            h_next = h[:, :, 1:, :, :]
...
class HunyuanVideo15ResnetBlock(nn.Cell):
    def __init__(
        self,
        in_channels: int,
...
class HunyuanVideo15MidBlock(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        num_layers: int = 1,
...
class HunyuanVideo15DownBlock3D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideo15UpBlock3D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideo15Encoder3D(nn.Cell):
    r"""
...
class HunyuanVideo15Decoder3D(nn.Cell):
    r"""
