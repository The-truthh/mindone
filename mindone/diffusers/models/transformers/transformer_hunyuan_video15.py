...
def chunked_logical_and(a, b, chunk_size=1000):
    batch, channel, h, w = a.shape
    result = mint.empty((batch, channel, h, w), dtype=ms.bool_)
    ...
    for i in range(0, h, chunk_size):
        ...
        for j in range(0, w, chunk_size):
            ...
            b_chunk = b[:, :, i:end_i, j:end_j]

            result[:, :, i:end_i, j:end_j] = mint.logical_and(a_chunk, b_chunk)
...
class HunyuanVideo15AttnProcessor2_0:
    ...
    def __call__(
        ...
        # FIXME: & in mindspore has performance issue
        # attention_mask = (self_attn_mask_1 & self_attn_mask_2).bool()
        attention_mask = chunked_logical_and(self_attn_mask_1, self_attn_mask_2)
...
class HunyuanVideo15PatchEmbed(nn.Cell):
    def __init__(
        self,
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        hidden_states = hidden_states.flatten(2).swapaxes(1, 2)  # BCFHW -> BNC
        return hidden_states


class HunyuanVideo15AdaNorm(nn.Cell):
...
class HunyuanVideo15TimeEmbedding(nn.Cell):
    r"""
...
class HunyuanVideo15IndividualTokenRefinerBlock(nn.Cell):
    def __init__(
        self,
...
class HunyuanVideo15IndividualTokenRefiner(nn.Cell):
    def __init__(
        self,
        num_attention_heads: int,
        attention_head_dim: int,
...
class HunyuanVideo15TokenRefiner(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        num_attention_heads: int,
...
class HunyuanVideo15RotaryPosEmbed(nn.Cell):
...
class HunyuanVideo15ByT5TextProjection(nn.Cell):
...
class HunyuanVideo15ImageProjection(nn.Cell):
    def __init__(self, in_channels: int, hidden_size: int):
        super().__init__()
        self.norm_in = mint.nn.LayerNorm(in_channels)
        self.linear_1 = mint.nn.Linear(in_channels, in_channels)
        self.act_fn = mint.nn.GELU()
        self.linear_2 = mint.nn.Linear(in_channels, hidden_size)
        self.norm_out = mint.nn.LayerNorm(hidden_size)

    def construct(self, image_embeds: ms.Tensor) -> ms.Tensor:
...
class HunyuanVideo15TransformerBlock(nn.Cell):
    def __init__(
        self,
        num_attention_heads: int,
        attention_head_dim: int,
...
class HunyuanVideo15Transformer3DModel(
    ...
    def __init__(
        ...
        self.cond_type_embed = mint.nn.Embedding(3, inner_dim)
    ...
    def construct(
        ...
        batch_size, num_channels, num_frames, height, width = hidden_states.shape
        p_t, p_h, p_w = self.config_patch_size_t, self.config_patch_size, self.config_patch_size
        ...
        encoder_hidden_states_cond_emb = self.cond_type_embed(
            mint.zeros_like(encoder_hidden_states[:, :, 0], dtype=ms.long)
        )
        ...
        encoder_hidden_states_2_cond_emb = self.cond_type_embed(
            mint.ones_like(encoder_hidden_states_2[:, :, 0], dtype=ms.long)
        )
        ...
        is_t2v = mint.all(image_embeds == 0)
        if is_t2v:
            encoder_hidden_states_3 = encoder_hidden_states_3 * 0.0
            encoder_attention_mask_3 = mint.zeros(
                (batch_size, encoder_hidden_states_3.shape[1]), dtype=encoder_attention_mask.dtype
            )
        else:
            encoder_attention_mask_3 = mint.ones(
                (batch_size, encoder_hidden_states_3.shape[1]), dtype=encoder_attention_mask.dtype
            )
        encoder_hidden_states_3_cond_emb = self.cond_type_embed(
            2 * mint.ones_like(encoder_hidden_states_3[:, :, 0], dtype=ms.long)
        )
            ...
            new_encoder_hidden_states.append(
                mint.cat(
                    [
                        ...
                        mint.zeros_like(text_2[~text_mask_2]),  # invalid byt5 (zeroed)
                        mint.zeros_like(text[~text_mask]),  # invalid mllm (zeroed)
                    ],
                    dim=0,
                )
            )
