...
class ChronoEditRotaryPosEmbed(nn.Cell):
    def __init__(
        self,
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        if num_frames == 2:
            freqs_cos_f = (
                freqs_cos[0][: self.temporal_skip_len][[0, -1]].view(ppf, 1, 1, -1).broadcast_to((ppf, pph, ppw, -1))
            )
        else:
        ...
        if num_frames == 2:
            freqs_sin_f = (
                freqs_sin[0][: self.temporal_skip_len][[0, -1]].view(ppf, 1, 1, -1).broadcast_to((ppf, pph, ppw, -1))
            )
        else:
