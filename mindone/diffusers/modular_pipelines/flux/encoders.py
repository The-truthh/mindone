...
def encode_vae_image(vae: AutoencoderKL, image: ms.Tensor, generator: np.random.Generator, sample_mode="sample"):
...
class FluxTextEncoderStep(ModularPipelineBlocks):
    ...
    @staticmethod
    def _get_clip_prompt_embeds(components, prompt: Union[str, List[str]]):
        prompt = [prompt] if isinstance(prompt, str) else prompt
