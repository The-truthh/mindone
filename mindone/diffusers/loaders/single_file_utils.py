    ...
    def convert_flux2_double_stream_blocks(key: str, state_dict: dict[str, object]) -> None:
        ...
        if "double_blocks." in key:
            ...
            if "qkv" in within_block_name:
                ...
                state_dict[new_q_key] = Parameter(to_q_weight, name=new_q_key)
                state_dict[new_k_key] = Parameter(to_k_weight, name=new_k_key)
                state_dict[new_v_key] = Parameter(to_v_weight, name=new_v_key)
            else:
...
def convert_z_image_transformer_checkpoint_to_diffusers(checkpoint, **kwargs):
    ...
    def convert_z_image_fused_attention(key: str, state_dict: dict[str, object]) -> None:
        ...
        state_dict[new_q_name] = Parameter(to_q_weight, name=new_q_name)
        state_dict[new_k_name] = Parameter(to_k_weight, name=new_k_name)
        state_dict[new_v_name] = Parameter(to_v_weight, name=new_v_name)
        return
