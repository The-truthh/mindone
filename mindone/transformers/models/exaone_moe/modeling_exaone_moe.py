...
class ExaoneMoeRMSNorm(ms.nn.Cell):
...
class ExaoneMoeAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        cache_position: ms.Tensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple[ms.Tensor, ms.Tensor | None, tuple[ms.Tensor] | None]:
...
class ExaoneMoeTopkRouter(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.weight = ms.Parameter(mint.empty((config.num_experts, config.hidden_size)))
        self.register_buffer("e_score_correction_bias", mint.zeros(config.num_experts))

    def construct(self, hidden_states):
        hidden_states = hidden_states.view(-1, self.config.hidden_size)
        router_logits = mint.nn.functional.linear(hidden_states.to(ms.float32), self.weight.to(ms.float32))
        return router_logits


class ExaoneMoeExperts(ms.nn.Cell):
...
class ExaoneMoeSparseMoEBlock(ms.nn.Cell):
    """
    ...
    def route_tokens_to_experts(self, router_logits):
        ...
        group_mask = mint.nn.functional.one_hot(group_idx, num_classes=self.n_group).sum(dim=1).to(group_scores.dtype)
        score_mask = (
...
class ExaoneMoeDecoderLayer(GradientCheckpointingLayer):
    ...
    def construct(
        ...
        cache_position: ms.Tensor | None = None,
        position_embeddings: tuple[ms.Tensor, ms.Tensor] | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> ms.Tensor:
...
class ExaoneMoeRotaryEmbedding(ms.nn.Cell):
...
class ExaoneMoeForCausalLM(ExaoneMoePreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        use_cache: bool | None = None,
        cache_position: ms.Tensor | None = None,
        logits_to_keep: int | ms.Tensor = 0,
        **kwargs: Unpack[TransformersKwargs],
