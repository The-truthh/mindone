...
class Swinv2Embeddings(nn.Cell):
    ...
    def __init__(self, config, use_mask_token=False):
        ...
        self.mask_token = ms.Parameter(mint.zeros((1, 1, config.embed_dim)), name="mask_token") if use_mask_token else None

        if config.use_absolute_embeddings:
            self.position_embeddings = ms.Parameter(mint.zeros((1, num_patches + 1, config.embed_dim)), name="position_embeddings")
        else:
