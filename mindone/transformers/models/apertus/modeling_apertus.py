...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = mint.nn.Softmax(dim=-1)(attn_weights.astype(ms.float32)).astype(query.dtype)
...
class ApertusAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        query_states = self.q_proj(hidden_states).reshape(hidden_shape).swapaxes(1, 2)
        key_states = self.k_proj(hidden_states).reshape(hidden_shape).swapaxes(1, 2)
        value_states = self.v_proj(hidden_states).reshape(hidden_shape).swapaxes(1, 2)
