...
class ApertusRMSNorm(ms.nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        hidden_states = ms.ops.Cast()(hidden_states, ms.float32)
        variance = mint.pow(hidden_states, 2).mean(-1, keepdims=True)
        hidden_states = hidden_states * mint.rsqrt(variance + self.variance_epsilon)
        return self.weight * ms.ops.Cast()(hidden_states, input_dtype)
...
class ApertusRotaryEmbedding(ms.nn.Cell):
    ...
    def construct(self, x, position_ids):
        inv_freq_expanded = ms.ops.ExpandDims()(ms.ops.ExpandDims()(ms.ops.Cast()(self.inv_freq, ms.float32), 0), -1)
        inv_freq_expanded = ms.ops.BroadcastTo((position_ids.shape[0], -1, 1))(inv_freq_expanded)
        position_ids_expanded = ms.ops.ExpandDims()(ms.ops.Cast()(position_ids, ms.float32), 1)

        # MindSpore doesn't need autocast, operations default to float32
        freqs = ms.ops.matmul(inv_freq_expanded, position_ids_expanded).transpose(1, 2)
        emb = mint.cat((freqs, freqs), dim=-1)
        cos = mint.cos(emb) * self.attention_scaling
        sin = mint.sin(emb) * self.attention_scaling

        return ms.ops.Cast()(cos, x.dtype), ms.ops.Cast()(sin, x.dtype)
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    """
    cos = ms.ops.ExpandDims()(cos, unsqueeze_dim)
    sin = ms.ops.ExpandDims()(sin, unsqueeze_dim)
...
def repeat_kv(hidden_states: ms.Tensor, n_rep: int) -> ms.Tensor:
    ...
    hidden_states = ms.ops.ExpandDims()(hidden_states, 2)
    hidden_states = ms.ops.BroadcastTo((batch, num_key_value_heads, n_rep, slen, head_dim))(hidden_states)
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = mint.softmax(ms.ops.Cast()(attn_weights, ms.float32), dim=-1)
    attn_weights = ms.ops.Cast()(attn_weights, query.dtype)
    attn_weights = ms.ops.Dropout(p=dropout)(attn_weights, training=module.training)
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if position_ids is None:
            position_ids = ms.ops.ExpandDims()(cache_position, 0)
