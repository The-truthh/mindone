...
def normalize(image, mean, std):
    mean = ms.tensor(mean, dtype=image.dtype).view(1, -1, 1, 1)
    std = ms.tensor(std, dtype=image.dtype).view(1, -1, 1, 1)
...
def rescale_and_normalize(
    ...
    if do_normalize:
        images = normalize(images.to(dtype=ms.float32), image_mean, image_std)
    elif do_rescale:
...
class OpenPanguVLImageProcessorFast(Qwen2VLImageProcessorFast):
    ...
    def _process_images(self, images, params, input_data_format, disable_grouping, return_tensors):
        ...
        data = self._preprocess(
                ...
                size=params.size,
                interpolation=pil_mindspore_interpolation_mapping.get(params.resample, params.resample),
