...
from .image_processing_openpangu_vl_fast import OpenPanguVLImageProcessorFast, rescale_and_normalize
...
class OpenPanguVisionPatchEmbed(ms.nn.Cell):
    def __init__(
        self,
        patch_size: int = 14,
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        if hidden_states.shape[-1] != self.input_size:
            hidden_states = mint.cat([hidden_states.reshape(-1, self.patch_size * self.patch_size), \
...
class OpenPanguVisionRotaryEmbedding(ms.nn.Cell):
...
class OpenPanguVLPatchMerger(ms.nn.Cell):
...
class OpenPanguVLVisionAttention(ms.nn.Cell):
...
class OpenPanguVisionTransformerPretrainedModel(OpenPanguPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = ms.nn.CellList([OpenPanguVLVisionBlock(config) for _ in range(config.depth)])
    ...
    def get_window_index(self, grid_thw):
        ...
        for grid_t, grid_h, grid_w in grid_thw:
            ...
            index_padded = mint.nn.functional.pad(index, (0, pad_w, 0, pad_h), "constant", -100)
...
class OpenPanguVLRotaryEmbedding(ms.nn.Cell):
    ...
    def construct(self, x, position_ids):
        ...
        if self.mrope_interleaved:
            ...
            emb = mint.cat(
                [m[self.mrope_dim[i]] for i, m in enumerate(emb.split(mrope_section_3d, dim=-1))], dim=-1
            )
        cos = emb.cos() * self.attention_scaling
        sin = emb.sin() * self.attention_scaling
        if not self.mrope_interleaved and self.mrope_section:
            mrope_section = self.mrope_section * 2
            cos = mint.cat([m[i % 3] for i, m in enumerate(cos.split(mrope_section, dim=-1))], dim=-1)
            sin = mint.cat([m[i % 3] for i, m in enumerate(sin.split(mrope_section, dim=-1))], dim=-1)
...
class OpenPanguVLAttention(ms.nn.Cell):
    """
...
class ProjectionSingle(ms.nn.Cell):
    def __init__(self, i_hidden_size: int, t_hidden_size: int):
        super().__init__()
        self.act = mint.nn.functional.silu
        self.fc1 = mint.nn.Linear(i_hidden_size, t_hidden_size, bias=True)  # bias 默认为 True
...
class OpenPanguVLModel(OpenPanguPreTrainedModel):
    ...
    def _parse_preprocess_params(self, vision_config):
        ...
        self.do_rescale = getattr(vision_config, "do_rescale", OpenPanguVLImageProcessorFast.do_rescale)
        self.rescale_factor = getattr(vision_config, "rescale_factor", OpenPanguVLImageProcessorFast.rescale_factor)
        self.do_normalize = getattr(vision_config, "do_normalize", OpenPanguVLImageProcessorFast.do_normalize)
        self.image_mean = tuple(getattr(vision_config, "image_mean", OpenPanguVLImageProcessorFast.image_mean))
        self.image_std = tuple(getattr(vision_config, "image_std", OpenPanguVLImageProcessorFast.image_std))
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                while idx < len(src_item):
                    ...
                    if src_item[idx] not in [video_token_id, image_token_id]:
                        ...
                        llm_pos_ids = ms.Tensor([start_idx], dtype=ms.int64).expand(3, -1)
                        ...
                        t_index = (mint.arange(grid_t) * 1 * tokens_per_second).long()
                        ...
                        for t in range(T):
                            if t != 0:
                                ...
                                bot_pos = ops.full((3, 1), current_pos, dtype=ms.int64)
                                llm_pos_ids_list.append(bot_pos)
                                current_pos += 1
                            
                            grid_h = mint.arange(llm_H).view(-1, 1).expand(-1, llm_W).flatten()
                            grid_w = mint.arange(llm_W).view(1, -1).expand(llm_H, -1).flatten()
                            
                            frame_pos = mint.stack(
                                [
                                    mint.full_like(grid_h, 0, dtype=ms.int64),      # t
                                    grid_h,                                    # h
                                    grid_w                                     # w
                                ]
                            ) 
                            ...
                            if t != final_frame_time:
                                ...
                                eot_pos = ops.full((3, 1), current_pos, dtype=ms.int64)
...
OpenPanguVLForConditionalGeneration = OpenPanguVL
OpenPanguVLPreTrainedModel = OpenPanguPreTrainedModel
