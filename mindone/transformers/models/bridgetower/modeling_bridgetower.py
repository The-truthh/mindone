    ...
    def __init__(self, config):
        ...
        if not config.share_layernorm:
            self.ln_separate = nn.CellList(
                [mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps) for _ in range(config.num_hidden_layers)]
            )
...
class BridgeTowerForContrastiveLearning(BridgeTowerPreTrainedModel):
    ...
    def construct(
        ...
        image_token_type_embeddings = self.bridgetower.token_type_embeddings(mint.full((1,), 1, dtype=ms.int64)).expand_as(image_embeds_with_ln)
