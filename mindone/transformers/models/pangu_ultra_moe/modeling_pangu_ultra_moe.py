...
class PanguUltraMoERMSNorm(nn.Cell):
    ...
    def construct(self, input_x):
        ...
        var = input_x.to(ms.float32).pow(2).mean(-1, keepdim=True)
        input_x = input_x * mint.rsqrt(var + self.epsilon)
...
class PanguUltraMoERotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, kv_len, max_seq_len=None):
        seq_len = max_seq_len if max_seq_len is not None else kv_len
...
class MoEGate(nn.Cell):
    def __init__(self, config):
        ...
        self.weight = ms.Parameter(
            mint.empty((config.num_routed_experts, config.hidden_size))
        )
    ...
    def construct(self, hidden_states):
        ...
        logits = mint.nn.functional.linear(
            hidden_states.to(ms.float32), self.weight.to(ms.float32), None
        )
        scores = logits.sigmoid()
        scores_for_choice = scores.view((bsz * seq_len, -1))
        _, topk_idx = mint.topk(scores_for_choice, k=self.top_k, dim=-1, sorted=False)
...
class PanguUltraMoE(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        final_hidden_states = mint.zeros_like(cat_hidden_states).scatter(
            0, idxs.unsqueeze(-1).expand(cat_hidden_states.shape), cat_hidden_states
        )
...
class PanguUltraMoEAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        k_nope, value = mint.split(
            kv, [self.attention_qk_dim, self.attention_v_dim], dim=-1
        )
        query = mint.cat((q_nope, q_pe), dim=-1)
        if k_pe.shape[1] != self.num_heads:
            k_pe = k_pe.expand((bsz, self.num_heads, q_len, self.attention_qk_rope_dim))
        ...
        attn_output = attn_output.transpose(1, 2).view((bsz, q_len, -1))
