...
from transformers.models.perception_lm.configuration_perception_lm import PerceptionLMConfig
...
class PerceptionLMAdaptiveAvgPooling(nn.Cell):
...
class PerceptionLMMultiModalProjector(nn.Cell):
...
class PerceptionLMModelOutputWithPast(BaseModelOutputWithPast):
    ...
    image_hidden_states (`ms.Tensor`, *optional*):
        A `ms.FloatTensor` of size `(batch_size, num_images, sequence_length, hidden_size)`.
    ...
    video_hidden_states (`ms.Tensor`, *optional*):
        A `ms.Tensor` of size `(batch_size, num_videos, sequence_length, hidden_size)`.
...
class PerceptionLMModel(PerceptionLMPreTrainedModel):
    ...
    def get_image_features(
        ...
        **kwargs,
        ...
        Args:
            pixel_values (`ms.Tensor]` of shape `(batch_size, num_tiles, channels, height, width)`)
               The tensors corresponding to the input images.
        Returns:
            image_features (`ms.Tensor`): Image feature tensor of shape `(num_tiles, num_patches, embed_dim)`).
        """
    ...
    def check_mask_feature_size_match(self, media_mask, media_features):
        ...
        media_feature_size = media_features.shape[:-1].numel()
...
class PerceptionLMForConditionalGeneration(PerceptionLMPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **lm_kwargs,
        ...
        >>> from mindone.transformers import PerceptionLMForConditionalGeneration
