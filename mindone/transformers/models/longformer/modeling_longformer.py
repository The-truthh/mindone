...
def as_strided(input_tensor, size, stride, storage_offset=0):
    input_nelement = input_tensor.nelement()

    flat_input = input_tensor.contiguous().view(-1)

    grid = mint.meshgrid(*[mint.arange(dim_size) for dim_size in size], indexing='ij')

    # 计算每个位置在展平输入中的索引
    # 公式: storage_offset + sum(i * stride_i for each dimension i)
    indices = mint.zeros(size, dtype=ms.int64)
    for i, coord in enumerate(grid):
        indices += coord * stride[i]
    indices += storage_offset

    # 检查是否有索引超出范围
    if mint.any(indices < 0) or mint.any(indices >= input_nelement):
        raise ValueError("某些索引超出了输入张量的范围")

    # 使用索引从展平输入中提取数据
    result = flat_input[indices]
    ...
    def construct(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        if layer_head_mask is not None:
            assert layer_head_mask.shape == (self.num_heads,), (
    ...
    def _chunk(hidden_states, window_overlap, onnx_export: bool = False):
        ...
        if not onnx_export:
            ...
            hidden_states = hidden_states.view(
                ...
                mint.div(hidden_states.shape[1], (window_overlap * 2), rounding_mode="trunc").item(),
                window_overlap * 2,
...
class LongformerEncoder(nn.Cell):
    ...
    def construct(
        ...
        if head_mask is not None:
            assert head_mask.shape[0] == (len(self.layer)), (
