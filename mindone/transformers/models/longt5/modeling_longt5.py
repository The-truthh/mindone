...
def _make_global_fixed_block_ids(
    attention_mask: ms.Tensor, global_block_size: int
    ...
    if num_globals > 0:
        _sequence_block_ids_max = mint.max(global_block_ids.float(), dim=-1)[0].int().tile((num_globals, 1)).swapaxes(0, 1)
    else:
...
def _create_global_aggregates(
    hidden_states: ms.Tensor, block_ids: ms.Tensor, global_seq_len: int
) -> ms.Tensor:
