...
def dynamic_rope_update(rope_forward):
    ...
    def longrope_frequency_update(self, position_ids, layer_type=None):
    ...
    def dynamic_frequency_update(self, position_ids, layer_type=None):
        """
    ...
    def wrapper(self, x, position_ids, layer_type=None):
        ...
        if "dynamic" in rope_type:
            dynamic_frequency_update(self, position_ids, **kwargs)
        elif rope_type == "longrope":
            longrope_frequency_update(self, position_ids, **kwargs)
...
def _compute_dynamic_ntk_parameters(
    ...
    if seq_len is None:
        ...
        seq_len = mint.maximum(
            seq_len,
            ms.Tensor(config.max_position_embeddings, dtype=seq_len.dtype, ),
        )
    else:
...
def _compute_yarn_parameters(
    ...
    pos_freqs = base ** (mint.arange(0, dim, 2).to(dtype=ms.float32) / dim)
    ...
    inv_freq_extrapolation_factor = 1 - linear_ramp_factor(low, high, dim // 2).to(dtype=ms.float32)
    inv_freq = (
