...
class DiTPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_dit_256(self, mode, dtype):
        ...
        pipe_cls = get_module("mindone.diffusers.pipelines.dit.DiTPipeline", mindspore_dtype=ms_dtype)
        pipe = pipe_cls.from_pretrained("facebook/DiT-XL-2-256")

        words = ["vase", "umbrella", "white shark", "white wolf"]
        ids = pipe.get_label_ids(words)

        torch.manual_seed(0)
        images = pipe(ids, num_inference_steps=40, output_type="np")[0]
