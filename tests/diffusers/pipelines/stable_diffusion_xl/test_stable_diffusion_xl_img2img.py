...
class StableDiffusionXLImg2ImgPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_stable_diffusion_xl_img2img_default_case(self, mode, dtype):
        ...
        pipe = pipe_cls.from_pretrained(
            "stabilityai/stable-diffusion-xl-refiner-1.0", mindspore_dtype=ms_dtype
        )
        ...
        torch.manual_seed(0)
        image = pipe(prompt, image=init_image, output_type="np")[0][0]
        ...
        threshold = THRESHOLD_FP32 if dtype == "float32" else THRESHOLD_FP16
        assert np.max(np.linalg.norm(expected_image - image) / np.linalg.norm(expected_image)) < threshold
