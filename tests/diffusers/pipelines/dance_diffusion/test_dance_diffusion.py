...
@ddt
class PipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_dance_diffusion(self, mode, dtype):
        ms.set_context(mode=mode)
        ...
        pipe = pipe_cls.from_pretrained("harmonai/maestro-150k", mindspore_dtype=ms_dtype)
        pipe.set_progress_bar_config(disable=None)

        torch.manual_seed(0)
        output = pipe(num_inference_steps=100, audio_length_in_s=4.096)
        audio = output[0]
