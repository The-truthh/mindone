...
@ddt
class WuerstchenPriorPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_wuerstchen_prior(self, mode, dtype):
        ms.set_context(mode=mode)
        ...
        pipe_cls = get_module("mindone.diffusers.pipelines.wuerstchen.WuerstchenPriorPipeline")
        prior_pipe = pipe_cls.from_pretrained("warp-ai/wuerstchen-prior", mindspore_dtype=ms_dtype)

        prompt = "an image of a shiba inu, donning a spacesuit and helmet"
        torch.manual_seed(0)
        prior_output = prior_pipe(prompt, output_type="np")[0]

        expected_image = load_downloaded_numpy_from_hf_hub(
            "The-truth/mindone-testing-arrays",
            f"wuerstchen_prior_{dtype}.npy",
            subfolder="wuerstchen",
        )
        threshold = THRESHOLD_FP32 if dtype == "float32" else THRESHOLD_FP16
        assert np.max(np.linalg.norm(expected_image - prior_output) / np.linalg.norm(expected_image)) < threshold
