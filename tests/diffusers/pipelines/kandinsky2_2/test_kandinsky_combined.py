...
@ddt
class KandinskyV22PipelineCombinedFastTests(unittest.TestCase):
    ...
    def test_kandinsky(self, mode, dtype):
        ...
        assert np.abs(pt_image_slice - ms_image_slice).max() < threshold


@ddt
class KandinskyV22PipelineImg2ImgCombinedFastTests(unittest.TestCase):
    def get_dummy_components(self):
    ...
    def get_dummy_inputs(self, seed=0):
        prior_dummy = PriorDummies()
        dummy = Img2ImgDummies()
        pt_prior_inputs = ms_prior_inputs = prior_dummy.get_dummy_inputs(seed=seed)
        pt_inputs, ms_inputs = dummy.get_dummy_inputs(seed=seed)

        pt_inputs.update(pt_prior_inputs)
        ms_inputs.update(ms_prior_inputs)

        pt_inputs.pop("image_embeds")
        pt_inputs.pop("negative_image_embeds")
        ms_inputs.pop("image_embeds")
        ms_inputs.pop("negative_image_embeds")
...
@ddt
class KandinskyV22PipelineInpaintCombinedFastTests(unittest.TestCase):
    def get_dummy_components(self):
