...
class KandinskyV22PipelineCombinedIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_kandinsky(self, mode, dtype):
        ...
        torch.manual_seed(0)
        image = pipe(prompt=prompt, num_inference_steps=25, output_type="np")[0][0]
    ...
    def test_kandinsky_inpaint(self, mode, dtype):
        ...
        torch.manual_seed(0)
        image = pipe(
            prompt=prompt, image=original_image, mask_image=mask, num_inference_steps=25, output_type="np"
        )[0][0]
